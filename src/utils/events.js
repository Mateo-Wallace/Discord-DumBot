...
export async function events() {
  ...
  player.events.on('playerPause', async (_queue, _track) => {
  ...
  player.events.on('audioTrackAdd', (_queue, _track) => {
  ...
  player.events.on('audioTracksAdd', (queue, _tracks) => {
    queue.metadata.channel.send(
      'All the songs in the playlist have been added to the queue ✅',
    );
  });

  player.events.on('connection', (_queue) => {
